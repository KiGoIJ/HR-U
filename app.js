...
// SHA-256 hash of custom master key
...
async function verifyMasterKey(input) {
  ...
  const saltedHash = await hashPassword(trimmed, DEFAULT_MASTER_SALT);

  // Raw SHA-256 (in case generated by online generator without salt)
  const enc = new TextEncoder();
  const rawBuf = await crypto.subtle.digest("SHA-256", enc.encode(trimmed));
  const rawHash = Array.from(new Uint8Array(rawBuf))
    .map((b) => b.toString(16).padStart(2, "0"))
    .join("");

  return saltedHash === MASTER_HASH || rawHash === MASTER_HASH;
}
...
async function saveUser(data, userId = null) {
  ...
  if (!userId) {
    ...
    const key = existing._dbKey || existing.id || userId;
    await R.users.child(key).update(updateObj);
...
async function deleteUser(userId) {
  ...
  const key = u._dbKey || u.id || userId;
  await R.users.child(key).remove();
  if (u.id && u.id !== key) {
    await R.users.child(u.id).remove().catch(() => {});
  }
  ...
  if (Array.isArray(val)) {
    val.forEach((u, i) => {
      if (u && typeof u === "object") {
        const id = u.id || "usr_idx_" + i;
        map[id] = { ...u, id, _dbKey: String(i) };
      }
    });
    return map;
  }
  ...
  R.users.on(
    ...
    async (s) => {
      ...
      if (Array.isArray(raw)) {
        ...
        await R.users.set(migrated).catch(console.error);
        return;
      }
      state.users = normalizeUsers(raw);
      onSynced();
    },
    onErr
  );
